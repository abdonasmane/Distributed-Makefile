...
RESULTS_FILE="network_performance_results_IO.txt"
chunk_size=10000000  

# Initialize the results file
if [ ! -f "$RESULTS_FILE" ]; then
    echo "File_Size RTT(N) 1/To" > "$RESULTS_FILE"
fi
...
ssh "$SERVER_HOSTNAME" "cd ~/destination; java PongIO $PORT ./ $chunk_size &" &
server_pid=$!
...
echo "$LARGE_FILE_SIZE $average_timeRTTN $average_timeTo_1" >> "$RESULTS_FILE"


# Step 5: Cleanup - Stop the server on the remote machine
echo -e "${CYAN}Stopping the server on ${SERVER_HOSTNAME}...${NC}"
ssh "$SERVER_HOSTNAME" "pkill -f 'java PongIO $PORT ./ $chunk_size'"
